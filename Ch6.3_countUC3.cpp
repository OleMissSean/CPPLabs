...
{  
  ...
  getline(cin, userInput);	// Inputs userInput
	...
	while (count <= userInput.length()) // Sets loop to do whole string
	{
		...
		{
			letterCt++; // Adds 1 to the letterCt
			count++; // Adds 1 to the count
		}
		else
	...
	// Outputs amount of uppercase and lowercase letters in the string
	cout << "Amount of uppercase and lowercase letters added together " << letterCt << endl; 
